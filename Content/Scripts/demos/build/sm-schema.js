...
module.exports = schema => {
    ...
    const StateMachine_Node_Schema = {
        ...
        Delegate: {
            GetWidget: (GraphNode, ref) => {
                ...
                return React.createElement(StateMachineNode, { nodeName: 'StateMachineNode' });
            },
    ...
    schema.OnDetermineWiringStyle = [(OutputPin, InputPin, Params, DrawingPolicyContainer) => {
    ...
    schema.OnDetermineLinkGeometry = [(OutputPin, InputPin, StartWidgetGeometry, EndWidgetGeometry, Container) => {
    ...
    schema.OnUsingNodeWidgetMap = [() => true];

    schema.OnDrawPreviewConnector = [(PinGeometry, StartPoint, EndPoint, Pin, Params, Conatiner) => {
    ...
    schema.OnDrawSplineWithArrow = [(StartAnchorPoint, EndAnchorPoint, Params, DrawingPolicyContainer, ArrowRadius) => {
    ...
    schema.OnDrawSplineWithArrow_Geom = [(StartGeom, EndGeom, Params, DrawingPolicyContainer) => {
    ...
    schema.OnComputeSplineTangent = [(Start, End) => {
    ...
    /**********************
     ** SNodePin Delegate */
    schema.OnUsingDefaultPin = [() => true];

    schema.OnGetSlateBrushName = [bHovered => bHovered ? 'Graph.StateNode.Pin.BackgroundHovered' : 'Graph.StateNode.Pin.Background'];

    /********************
     ** SNode  Delegate */
    schema.OnUsingCustomContent = [node => true];

    schema.OnDisableMakePins = [node => node.IsTrainsionNode()];

    schema.OnSkipMoveTo = [node => node.IsTrainsionNode()];

    schema.OnRequiresSecondPassLayout = [node => node.IsTrainsionNode()];

    schema.OnPerformSecondPassLayout = [node => {
    ...
    schema.OnMouseEnter = [(node, slatenode) => {
        if (node.IsTrainsionNode()) {
            slatenode.AddPinToHoverSet(node.GetInputPin());
        }
    }];

    schema.OnMouseLeave = [(node, slatenode) => {
        if (node.IsTrainsionNode()) {
    ...
    /*************************
     ** Schema  Delegate */
    schema.OnTryCreateConnection = [(PinA, PinB) => {
